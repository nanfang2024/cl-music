        ...
        return try {
            val lyricId: String = when (track.source) {
                "netease" -> track.id
                    ...
                    val searchUrl = "$GD_API?types=search&source=${track.source}" +
                            "&name=${enc(track.name + " " + track.artist)}&count=3&pages=1"
                    val arr = JsonParser.parseString(get(searchUrl)).asJsonArray
                    var found: String? = null
                    for (el in arr) {
                        try {
                            val lid = el.asJsonObject["lyric_id"]
            ...
            val url = "$GD_API?types=lyric&source=${track.source}&id=${enc(lyricId)}"
            parse(get(url))?.get("lyric")?.takeIf { !it.isJsonNull }?.asString
        } catch (_: Exception) {
            null
        }
    }
